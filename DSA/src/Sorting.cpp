    ...
    for (std::size_t i = 0; i < sortedArray.size(); ++i) {
        ...
        for (std::size_t j = i + 1; j < sortedArray.size(); ++j) {
            ...
            if (sortedArray[j] < sortedArray[minIndex]) {
                ...
                logger.LogMessage(
                    std::format("New minimum index: {}", minIndex).c_str(),
                    ArgoDraft::LogLevel::DEBUG);
            }
        }
        ...
        logger.LogMessage(
            std::format("Swapping elements {} and {}", sortedArray[i], sortedArray[minIndex]).c_str(),
            ArgoDraft::LogLevel::INFO);
    }
...
void Sorting::insertionSort() const {
    ...
    for (std::size_t i = 1; i < sortedArray.size(); ++i) {
        ...
        while (j >= 0 && sortedArray[j] > key) {
            logger.LogMessage(
                std::format("Comparing elements {} and {}", sortedArray[j], key).c_str(),
                ArgoDraft::LogLevel::DEBUG);

            sortedArray[j + 1] = sortedArray[j];
            logger.LogMessage(
                std::format("Moving element {} to position {}", sortedArray[j], j + 1).c_str(),
                ArgoDraft::LogLevel::INFO);
        ...
        sortedArray[j + 1] = key;
        logger.LogMessage(
            std::format("Inserting key {} at position {}", key, j + 1).c_str(),
            ArgoDraft::LogLevel::INFO);
    }
